...
const Fire: FC<FireProps> = ({ show = false }) => {
  ...
  return (
    ...
    >
      <div
        className={`transition relative duration-1500 ${_show ? "opacity-85" : "opacity-0"} w-8 h-10 md:h-10 md:w-10 ${fireStyles.fire}`}
      >
        {[...Array(60).keys()].map((i) => (
          <div
            key={i}
            className={`transition duration-1500 h-10 w-8 md:h-10 md:w-12 ${fireStyles.particle}`}
          />
        ))}
      </div>
    </div>
