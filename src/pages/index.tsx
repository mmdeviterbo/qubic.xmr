...
const MiningStats: NextPage = () => {
  const [miningStats, setMiningStats] = useState<Record<string, number>>({});
  ...
  const formatLatestBlockFound = () => {
    const latestBlockFound = Number(`${miningStats?.last_block_found}000`);
      ...
      <div className="py-8 px-4 flex justify-center">
        <main className="w-full flex flex-col gap-16 lg:w-1/3 px-12 py-32">
          ...
          <Container
            ...
            value={miningStats?.connected_miners?.toLocaleString()}
            loading={isLoading}
          />
          
          <Container
            label={"Monero Network Hashrate"}
            value={miningStats?.network_hashrate?.toLocaleString()}
            loading={isLoading}
            customClass="mt-4"
          />
          <Container
            label={"Monero Network Height"}
            value={miningStats?.network_height?.toLocaleString()}
            loading={isLoading}
          />
          <Container
            label={"Monero Network Difficulty"}
            value={miningStats?.network_difficulty?.toLocaleString()}
            loading={isLoading}
          />

          <div className="flex mt-4 gap-4 text-gray-50 text-xs">
            <Link href={"https://xmr-stats.qubic.org/stats"}>Live data</Link>
          </div>
        </main>
      </div>
    </>
